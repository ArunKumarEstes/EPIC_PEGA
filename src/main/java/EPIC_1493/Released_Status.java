...
public class Released_Status extends UtilClass {
	...
	public void Released_Status_Update() throws JsonProcessingException, IOException, JSONException {
		String dirpath = System.getProperty("user.dir");
		File file = new File(dirpath + "//Status_Update.json");
		ObjectMapper mapper = new ObjectMapper();
		JsonNode node = mapper.readTree(file);
		...
		jsonObject.put("CaseID", sendkeys);
		jsonObject.put("StatusEvent", Status_Events[2]);
	...
	public Released_Status(WebDriver driver) {
...
//	public static WebElement pyWorkPage;
	...
	@FindBy(xpath = "//span[contains(text(), 'Resolved')]")
	...
	@FindBy(xpath = "//div[contains(text(),'Released')]")
...
//		System.out.println("Clipboard_ReceivedDate:" + attribute2);
		...
		waits(ReceivedDate_OrderPage_Clipboard);
...
//		assertEquals(dateActual, dateExpected);
		...
		assertTrue(text2.contains("Delivered"));
	...
	public void Released_Status_EPIC_Clipboard_validation() throws InterruptedException, ParseException {
...
//		pyWorkPage.click();	
		...
		String text = StatusEvent_OrderPage_Clipboard.getText();
		assertTrue(text.contains("Released"));
		...
		assertTrue(Status_Shipment2.contains("Released"));
...
//		frameswitch3();
		...
		Await();
		getIgnoringStaleElementException(Released_Status_in_Status_Datatype);
		String ReceivedStatus_Status_Datatype = Released_Status_in_Status_Datatype.getText();
		assertTrue(ReceivedStatus_Status_Datatype.contains("Released"));
		System.out
				.println("Successfully Validated Received_Status_in_Status_Datatype:" + ReceivedStatus_Status_Datatype);

		Await();
		getIgnoringStaleElementException(Status_Date_Time_in_Status_Datatype);
